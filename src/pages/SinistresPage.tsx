...
export default function SinistresPage() {
  ...
  const filtered = mockSinistres.filter(
    (s) =>
      ...
      <div className="space-y-4 max-w-7xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {Object.entries(statusConfig).map(([label, config]) => {
            const count = mockSinistres.filter((s) => s.statut === label).length;
            return (
        ...
        <div className="space-y-4">
          {filtered.map((sinistre, i) => {
            const config = statusConfig[sinistre.statut];
            return (
              <motion.div
                key={sinistre.id}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg flex items-center justify-center text-white font-bold">
                        {sinistre.assure.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
